...
public class RhizomeTests {
    ...
    public void testSimpleControllerGets() throws Exception {
        SimpleControllerAPI api = adapter.create( SimpleControllerAPI.class );
        // Use the project mapper (Jdk8Module + Guava + JavaTime + Kotlin registered); a bare
        // new ObjectMapper() can't serialize the Optional<ConnectorConfiguration> fields on
        // JettyConfiguration and throws InvalidDefinitionException.
    ...
    public static void plant() throws Exception {
        final String jwtToken = AuthenticationTest.createTestToken( "test-user" );
        rhizome = new Rhizome(
        ...
        OkHttpClient httpClient = new OkHttpClient.Builder()
                ...
                .addInterceptor( chain -> chain
                        .proceed( chain.request().newBuilder().addHeader( "Authorization", "Bearer " + jwtToken )
                                .build() ) )
                .build();
        int httpPort = rhizome.getContext()
                .getBean( JettyConfiguration.class )
                .getWebConnectorConfiguration()
                .orElseThrow()
                .getHttpPort();
        adapter = new Retrofit.Builder().baseUrl( "http://localhost:" + httpPort + "/rhizome/api/" ).client( httpClient )
