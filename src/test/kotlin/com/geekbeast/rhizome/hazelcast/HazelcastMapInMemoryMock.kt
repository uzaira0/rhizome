 ...
 */
// reason: test mock builder — one thenAnswer stub per IMap method, each closing over the same
// backing maps + generics; splitting would obscure the mock and duplicate captured state
@Suppress("LongMethod")
...
//    val valueStreamSerializer = getSerializers(ssMap, valueClass)
    ...
    if (ss is SerializationServiceV1) {
        registerStreamSerializers(ss, streamSerializers)
    }
    ...
    Mockito.`when`(mock.getAll(anySet())).thenAnswer { invocation ->
    ...
    Mockito.`when`(mock.keySet(any())).thenAnswer {
    ...
    Mockito.`when`(mock.values(any())).thenAnswer {
        val p = it.arguments[0] as Predicate<K, V>
        handleExpiration(backingMap, ttlMap)
        handleIdleness(backingMap, ttlMap, idleMap)
        backingMap.asSequence()
...
private fun registerStreamSerializers(
    ss: SerializationServiceV1,
    streamSerializers: List<SelfRegisteringStreamSerializer<*>>
) {
    try {
