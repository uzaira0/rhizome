...
class TaskService(
                ...
                .forEach { initializer ->
                    ...
                    if (initializer.isRunOnceAcrossCluster()) {
                        val urn = try {
                            ...
                            logger.info("Duplicate task registration detected.", ex)
    ...
    // reason: boundary catch — one task's scheduling failure must not abort scheduling of the rest at startup
    @Suppress("TooGenericExceptionCaught")
    private fun scheduleTask(task: HazelcastFixedRateTask<*>) {
        try {
    ...
    interface HazelcastDependencyAwareTask<T : HazelcastTaskDependencies> {
        ...
        fun getDependency(): T {
            startupLatch.await()
            return (dependencies[getDependenciesClass()] ?: error(
    ...
    private class InitializersCompletedRequirements : Requirement {
        ...
        // reason: boundary catch — startup requirement reports unsatisfied on any await failure
        @Suppress("TooGenericExceptionCaught")
    ...
    class TaskComparator(
        ...
        override fun compare(a: HazelcastInitializationTask<*>, b: HazelcastInitializationTask<*>): Int {
            ...
            val mutualDependency = ancestorsOfA.contains(b.javaClass) && ancestorsOfB.contains(a.javaClass)
            val selfCycle = ancestorsOfA.contains(a.javaClass) || ancestorsOfB.contains(b.javaClass)
            return if (mutualDependency || selfCycle) {
                logger.error(
