...
class HazelcastJobService(hazelcastInstance: HazelcastInstance) {
    ...
    private val jobs = hazelcastInstance.getMap<UUID, AbstractDistributedJob<*, *>>(JOBS_MAP)
    private val durableExecutor = hazelcastInstance.getDurableExecutorService(JOBS_EXECUTOR)
    ...
    // reason: boundary catch — any (de)serialization failure means the job is invalid; both paths rethrow as IAE
...
// reason: Hazelcast Predicates.in is a vararg API requiring spread; predicate sets are small
