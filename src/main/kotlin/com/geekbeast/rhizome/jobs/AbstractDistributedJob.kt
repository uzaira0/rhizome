...
abstract class AbstractDistributedJob<R, S : JobState>(
    ...
    @get:SuppressFBWarnings(
            ...
            justification = "Kotlin lateinit getter never returns null: it throws " +
                    "UninitializedPropertyAccessException if accessed before setHazelcastInstance() " +
                    "assigns it. SpotBugs cannot model lateinit semantics and sees the nullable backing field."
    )
    ...
    internal fun initTaskId(taskId: Long) {
        this.taskId = if (this.taskId == null) taskId else error("Task id can only be assigned once.")
    }
    ...
    internal fun initId(id: UUID) {
        this.id = if (this.id == null) id else error("Job  id can only be assigned once.")
    }
    ...
    final override fun call(): R? {
        ...
        return if (hasWorkRemaining && status == JobStatus.CANCELED) {
            ...
            error("Distributed job $id is an illegal state.")
        }
    }
    ...
    private fun pollForTaskId() {
        ...
        while (taskId == null) {
            ...
            val maybeTaskId = jobs.executeOnKey(id!!) { it.value.taskId } // Future: Make offloadable
