        ...
        public static <T> T loadConfiguration( Class<T> clazz ) {
            ...
            if ( key == null ) {
                throw new IllegalStateException(
                        "Unable to load configuration key for class " + clazz.getName()
                        + ". Ensure the class has a static key() method or @ReloadableConfiguration annotation." );
            }

            T config = loadConfigurationFromResource( key, clazz );
            if ( config == null ) {
                throw new IllegalStateException(
                        "Failed to load configuration for " + clazz.getName()
                        + " from resource " + key.getUri()
